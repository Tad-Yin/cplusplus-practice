...
#include <cassert>
namespace tad {
    void svpng(const std::string path,
            const std::vector<std::vector<unsigned char>> data, Alpha alpha) {
        ...
        uint32_t crc, a = 1, b = 0, p = w * (alpha == Alpha::haveAlpha ? 4 : 3);

        auto u32 = [&of](uint32_t u) {of.put(u >> 24);of.put((u >> 16) & 255);of.put((u >> 8) & 255);of.put(u & 255);};
        auto u8c = [&of, &crc](char u){of.put(u);crc ^= u;crc = (crc >> 4) ^ t[crc & 15];crc = (crc >> 4) ^ t[crc & 15];};
        ...
        auto u8adler = [u8c, &a, &b](char u){u8c(u);a = (a + u) % 65521; b = (b + a) % 65521;};
